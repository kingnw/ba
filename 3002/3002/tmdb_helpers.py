...
# Cached functions for top-rated, new releases, and trending movies
...
# Cache for actor movie searches with memoization
...
# Search movies with caching and filters applied
@cache.memoize(timeout=1800)  # Cache each unique search for 30 minutes
def search_movie(query, filters=None):
    ...
    cache.set(cache_key, unique_movies, timeout=1800)  # 30-minute cache
...
# Fetch movies with a 5-minute cache
...
# Helper to process movie results
